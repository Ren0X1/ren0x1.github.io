...
export default function AdminPanel({ onToast }) {
  ...
  const [newUser, setNewUser] = useState({ name: '', email: '', pin: '1234', role: 'user' })
  ...
  const loadUsers = async () => {
    try {
      ...
      for (const p of profiles) {
        const cars = await getCars(p.id)
        counts[p.id] = cars.length
      }
      setCarCounts(counts)
    } catch (err) {
      onToast('Error cargando usuarios: ' + err.message, 'error')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      setNewUser({ name: '', email: '', pin: '1234', role: 'user' })
      setShowNew(false)
    ...
    <div style={css.container}>
      <div style={{ paddingTop: 28, paddingBottom: 40 }}>
              ...
              <div style={css.flexBetween}>
                <div>
                  ...
                  <p style={{ ...css.subtitle, marginTop: 4 }}>
                    {u.email} · PIN: {u.pin} · {carCounts[u.id] || 0} coche{(carCounts[u.id] || 0) !== 1 ? 's' : ''}
                  </p>
                </div>
                {u.role !== 'admin' && (
        ...
        <Modal open={showNew} onClose={() => setShowNew(false)} title="Nuevo Usuario">
          <Field label="Nombre">
            <input style={css.input} value={newUser.name} onChange={e => setNewUser(p => ({ ...p, name: e.target.value }))} placeholder="Nombre" />
          </Field>
          <Field label="Email">
            <input style={css.input} value={newUser.email} onChange={e => setNewUser(p => ({ ...p, email: e.target.value }))} placeholder="email@ejemplo.com" />
          </Field>
          <Field label="PIN">
            <input style={css.input} value={newUser.pin} onChange={e => setNewUser(p => ({ ...p, pin: e.target.value }))} placeholder="1234" />
          </Field>
          <div style={{ ...css.flex, justifyContent: 'flex-end', marginTop: 16, gap: 8 }}>
            ...
            <button onClick={handleAdd} disabled={saving} style={css.btn()}>
